...
@php
    ...
    $profile = ['label' => 'Akun', 'icon' => 'user', 'route' => 'profile.edit', 'match' => 'profile.*'];
    $user = auth()->user();
@endphp
...
<body class="min-h-screen bg-honeydew font-sans text-ultramarine antialiased">
        ...
        <div class="mt-auto space-y-1 border-t border-white/10 pt-4">
            <a href="{{ route('profile.edit') }}"
               class="flex items-center gap-3 rounded-xl p-2 transition hover:bg-white/10 {{ request()->routeIs('profile.*') ? 'bg-white/10' : '' }}">
                <span class="grid size-10 shrink-0 place-items-center rounded-full bg-periwinkle font-semibold text-ultramarine">{{ $user->initial() }}</span>
                <span class="min-w-0">
    ...
    {{-- Header (layar kecil) --}}
    <header class="bg-ultramarine px-4 py-3 lg:hidden">
    ...
    <main class="lg:pl-64">
        <div class="mx-auto max-w-4xl px-4 py-8 pb-28 sm:px-8 lg:pb-10">
            @if (session('success'))
                <div role="status" class="mb-6 rounded-xl border border-custard bg-custard/40 px-4 py-3 text-sm font-medium text-ultramarine">
                    {{ session('success') }}
                </div>
            @endif
    ...
    <nav class="fixed inset-x-0 bottom-0 z-20 flex bg-ultramarine pb-[env(safe-area-inset-bottom)] lg:hidden" aria-label="Menu utama">
        @foreach ([...$menu, $profile] as $item)
            @php $active = request()->routeIs($item['match']); @endphp
            <a href="{{ route($item['route']) }}" @if ($active) aria-current="page" @endif
               class="flex flex-1 flex-col items-center gap-1 py-2.5 text-[11px] font-medium transition {{ $active ? 'text-custard' : 'text-periwinkle' }}">
                <x-icon :name="$item['icon']" />
                {{ $item['label'] }}
            </a>
        @endforeach
